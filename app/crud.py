...
def get_chuyenxe_theo_tai_xe(db: Session, id: int, skip: int = 0, limit: int = 100):
    ...
    return db.query(models.Chuyen_xe).filter(models.association_table_taixe_chuyenxe.c.tai_xe_id== models.Tai_xe.id).filter(models.association_table_taixe_chuyenxe.c.chuyen_xe_id == models.Chuyen_xe.id).filter(models.Tai_xe.id==id).all()
